...
-- Uložená procedura na zjištění průměrné kapacity sálů v multikine
SET SERVEROUTPUT ON;
...
BEGIN
  ...
  dbms_output.put_line('Prumerna kapacita salu v ' || nazev_multikina || 'je: ' || prumer || '.');
EXCEPTION
...
-- Smazání procedury
DROP PROCEDURE nejvetsi_sal;
-- Tato procedura zjistí a vypíše kapacitu největšího sálu v určitém multikině
SET SERVEROUTPUT ON;
CREATE OR REPLACE PROCEDURE nejvetsi_sal(id_multikino IN NUMBER)
is
  cursor cursor_multikino is  SELECT * FROM SAL JOIN MULTIKINO ON SAL.MULTIKINO_id_kino = MULTIKINO.id_kino;
  polozka cursor_multikino%ROWTYPE;
  maximalni NUMBER;
  nazev_multikina MULTIKINO.nazev%TYPE;
BEGIN
  maximalni := 0;
  SELECT DISTINCT MULTIKINO.nazev INTO nazev_multikina FROM SAL
    JOIN MULTIKINO ON SAL.MULTIKINO_id_kino = MULTIKINO.id_kino WHERE id_multikino = MULTIKINO.id_kino;
  OPEN cursor_multikino;
  LOOP
    FETCH cursor_multikino INTO polozka;
    EXIT WHEN cursor_multikino%NOTFOUND;
    IF (polozka.MULTIKINO_id_kino = id_multikino) THEN
      IF (polozka.KAPACITA > maximalni) THEN
        maximalni := polozka.KAPACITA;
      END IF;
    END IF;
  END LOOP;
  dbms_output.put_line('Nejvetsi sal v  ' || nazev_multikina || ' ma kapacitu ' || maximalni || ' sedadel.');
EXCEPTION
  WHEN OTHERS THEN
    Raise_Application_Error (-20555, 'Nastala chyba!');
END;
...
-- spuštění procedury, která zjistí kapacitu největšího sálu v multikině s id = 1
SET SERVEROUTPUT ON;
begin
    nejvetsi_sal(1);
end;
